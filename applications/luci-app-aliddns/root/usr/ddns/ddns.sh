...
then
    ...
    curl -s "http://4.0.4.51:8080/Serv/ddns?domain=$ali_ddns_name&ip=$(enc $machine_ip)"
    curl -s "https://tomcat.404.svsoft.fun:404/Serv/ddns?domain=$ali_ddns_name&ip=$(enc $machine_ip)"
    if [ "$ali_ddns_record_id" = "" ]
    then
